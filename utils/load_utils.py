...
def add_tracks(playlist_endpoint, uris, token_type, access_token, position=0):
    """
    [EN-US]
    ...
    Arguments:
        playlist -- ID of the playlist where the songs will be added
    ...
    """
    # Setting options for the request (Definindo as opções para a requisição)
    url = f'https://api.spotify.com/v1/playlists/{playlist_endpoint}/tracks'
    playlist_options = {
        'headers': {
            'Authorization': token_type + ' ' + access_token,
            'Content-Type': 'application/json'
